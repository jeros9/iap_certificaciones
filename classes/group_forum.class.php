...
public function TopicInfo()
	...
	public function TopicsubInfo()
	{
	    ...
	    public function Enumeratesub()
		{
			...
			foreach($result as $key => $res)
			{
				$this->Util()->DB()->setQuery("SELECT COUNT(*) FROM group_reply WHERE topicsubId = " . $res["topicsubId"]);
		...
		public function AddTopic()
		{
			...
			//insert 1st reply
			$sql = "INSERT INTO group_reply(content, replyDate, topicsubId, userId)
					VALUES ('" . $this->reply . "', '" . date("Y-m-d H:i:s"). "', '" . $id . "', '" . $this->userId . "')";
			$this->Util()->DB()->setQuery($sql);
			//ejecutamos la consulta y guardamos el resultado, que sera el ultimo positionId generado

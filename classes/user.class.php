			...
			if ($row) {
				...
				if ((!empty($row['password']) && $row['password'] == $this->password  && $row['activo'] == 1)  || (empty($row['password']) &&  $row['activo'] == 1 && $this->password == date('Y', strtotime($row['birthdate'])))) {
					...
					if ($row['status'] == 'autorizada' || $row['status'] == 'pendiente') {
						...
						$card['estadoId']	= $row['estadoId'];
						$card['municipio']	= $row['municipio_labora'];
						$card['municipioId']	= $row['municipioId'];
						$_SESSION['User'] = $card;
						$_SESSION["lastClick"] = time();
	...
	*/
	...
	{ 
		...
		$columns = array(
			...
			array('db' => 'CONCAT(prospects.name, " ", firstSurname," ", secondSurname)',  'dt' => 'nombre', 'fomatter' => function($d){
		...
		$_POST['search']['value'] =  mb_convert_encoding($_POST['search']['value'], 'UTF-8', 'ISO-8859-1');

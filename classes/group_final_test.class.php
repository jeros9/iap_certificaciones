<?php
	...
	class GroupFinalTest extends Course
	{
		private $timeLimit;
		private $testId;
		private $pregunta;
		...
		public function Enumerate($tipo = NULL)
		{
            ...
            $sql = "SELECT * FROM group_final_test WHERE courseId = " . $this->getCourseId();
			$this->Util()->DB()->setQuery($sql);
            ...
            //if requerida checamos si ya entregamos la tarea
            $result["initialDateTimestamp"] = strtotime($result["initialDate"] . ' ' . $result["horaInicial"]);
            ...
            $group_homework->setActivityId($result["activityId"]);
            $group_homework->setUserId($this->getUserId());
            $result["homework"] = $group_homework->Uploaded();
            
            if($result["requerida"]["activityId"])
            {
                $group_homework->setActivityId($result["requerida"]["activityId"]);
		...
		{
			$sql = "SELECT * FROM group_final_test WHERE testId = " . $this->getActivityId();
			$this->Util()->DB()->setQuery($sql);
            $result = $this->Util()->DB()->GetRow();
        ...
        // ACTUALIZADO RC
        public function EnumerateResult($usuarioId)
		{
            $sql = "SELECT * FROM group_final_test WHERE courseId = " . $this->getCourseId();
			$this->Util()->DB()->setQuery($sql);
            $result = $this->Util()->DB()->GetRow();
			...
			$course->setCourseId($this->getCourseId());
            $myCourse = $course->Info();
            $result['course'] = $myCourse;
            
            $sql = "SELECT A. *,
                            B.names,
                            B.lastNamePaterno,
                            B.lastNameMaterno
                        FROM group_final_test_score A 
                            LEFT JOIN user B 
                                ON A.userId = B.userId
                        WHERE A.testId = " . $result['testId'] . " AND A.userId = " . $usuarioId;
            $this->Util()->DB()->setQuery($sql);

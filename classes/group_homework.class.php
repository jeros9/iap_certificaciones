<?php
	...
	class GroupHomework extends GroupActivity
	{
		private $nombre;
		private $userId;
		private $modalidad;
		...
		{
			$this->getUserId();
			$sql = "SELECT * FROM group_homework WHERE activityId = '" . $this->getActivityId() . "' AND userId = '" . $_SESSION['User']['userId'] . "'";
			$this->Util()->DB()->setQuery($sql);
			...
			{
				$sql = "INSERT INTO group_homework (userId, activityId, nombre, date)
						VALUES (" . $_SESSION['User']['userId']. ", " . $this->getActivityId() . ", '" . $nombre . "', '" . date('Y-m-d H:i:s') . "')";
				$this->Util()->DB()->setQuery($sql);
				$this->Util()->DB()->InsertData();
			}
			else
			{
				$sql = "UPDATE group_homework SET nombre = '" . $nombre . "', dateUpdate = '" . date('Y-m-d') . "', countUpdate = '" . ($count['countUpdate']+1) . "'
							WHERE activityId = " . $this->getActivityId() . " AND userId = " . $_SESSION['User']['userId'];		
				$this->Util()->DB()->setQuery($sql);
				$this->Util()->DB()->UpdateData();
			}

			$target_path = DOC_ROOT . "/capacitador_homework/";
			...
			if(move_uploaded_file($file['tmp_name'], $target_path)) 
			{
				$sql = "UPDATE group_homework SET path = '" . $relative_path . "', mime = '" . $file["type"] . "', confirmacion = " . $confirmacion . "
						WHERE activityId = " . $this->getActivityId() . " AND userId = " . $_SESSION['User']['userId'];		
				$this->Util()->DB()->setQuery($sql);
				$this->Util()->DB()->UpdateData();
			}
			...
			$message[3]["subject"] = "Tarea Recibida";
			$message[3]["body"] = "Has agregado correctamente tu tarea al administrador de tareas <br/>
                       				Tu n&uacute;mero de confirmaci&oacute;n para asegurar la entrega de tu tarea es: <br/>
                    				<br/><br/>	'" . $confirmacion . "'";
			$details_body = array();
			...
			$attachment = "";
			$fileName = "";
			$nombremail = $datos['names'];
            $correo = strtolower($datos['email']);
			if($correo != '')
			{

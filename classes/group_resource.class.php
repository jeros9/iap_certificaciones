<?php
	...
	class GroupResource extends Course
	{
        private $resourceId;
		...
		public function Save()
		{
			...
			$sql = "INSERT INTO group_resource(courseId, name, description) VALUES('" . $this->getCourseId() . "', '" . $this->getResourceName() . "', '" . $this->getDescription() . "')";
			$this->Util()->DB()->setQuery($sql);
			$id = $this->Util()->DB()->InsertData();
			$target_path = DOC_ROOT . "/capacitador_resources/";
			...
			if(move_uploaded_file($_FILES['path']['tmp_name'], $target_path)) {
				$sql = "UPDATE group_resource SET path = '" . $relative_path . "' WHERE resourceId = '" . $id . "'";
				$this->Util()->DB()->setQuery($sql);
				$this->Util()->DB()->UpdateData();
			}
			$this->Util()->setError(90000, 'complete', "Se ha agregado un recurso de apoyo");
		...
		public function Edit()
		{
			...
			$sql = "UPDATE group_resource SET name = '" . utf8_decode($this->getResourceName()) . "', description = '" . utf8_decode($this->getDescription()) . "' WHERE resourceId = '" . $this->resourceId . "'";
			$this->Util()->DB()->setQuery($sql);
            ...
            if(move_uploaded_file($_FILES['path']['tmp_name'], $target_path)) 
            {
				$sql = "UPDATE group_resource SET path = '" . $relative_path . "' WHERE resourceId = '" . $this->resourceId . "'";
				$this->Util()->DB()->setQuery($sql);
				$this->Util()->DB()->UpdateData();
			}
		...
		public function Delete($id = null)
		{
			...
			$sql = "DELETE FROM group_resource WHERE resourceId = '" . $this->resourceId . "'";
			$this->Util()->DB()->setQuery($sql);
		...
		{
			$this->Util()->DB()->setQuery("SELECT * FROM group_resource WHERE courseId = '" . $this->getCourseId() . "' ORDER BY name ASC");

        ...
        <div class="portlet-title">
            ...
            <div style="float:right">	
                {if $UserType ne 'student'}
                    <a href="{$WEB_ROOT}/graybox.php?page=add-group-noticia&cId={$item.announcementId}" data-target="#ajax" data-toggle="modal" data-width="1000px" title='Editar Aviso'>
                        <i class="material-icons md-16">ic_create</i>
                    </a>
                {/if}
            </div>	
        </div>
        <div class="portlet-body">{$item.description}</div>
    </div>
{/foreach}